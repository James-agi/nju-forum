...
if (!/^[A-Za-z0-9_-]{64}$/.test(secret)) {
  throw new Error("NEXTAUTH_SECRET must be a 64-character base64url value");
}
...
const currentLines = fs
  ...
  .split(/\r?\n/)
  .filter((line) => !/^\s*(?:export\s+)?NEXTAUTH_SECRET\s*=/.test(line));
...
process.stdout.write("NEXTAUTH_SECRET_UPDATED\n");
