...
| `KNOWLEDGE_ANSWER_API_KEY` | 最终回答专用 LLM API Key | → `KNOWLEDGE_LLM_API_KEY` → `OPENAI_API_KEY` |
| `KNOWLEDGE_ANSWER_MODEL` | 最终回答专用 LLM 模型 | → `KNOWLEDGE_LLM_MODEL` |
| `KNOWLEDGE_ANSWER_BASE_URL` | 最终回答专用 LLM API 地址 | → `KNOWLEDGE_LLM_BASE_URL` → `OPENAI_BASE_URL` |
| `KNOWLEDGE_ANSWER_TIMEOUT_MS` | 最终回答专用 LLM 超时 | `25000`，限制在 5000-90000 |
